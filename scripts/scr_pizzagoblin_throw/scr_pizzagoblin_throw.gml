...
{
    hsp = 0;
    sprite_index = throw_spr;
    image_speed = throwspd;
    ...
    if (bombreset <= 0 && floor(image_index) == throw_frame)
    {
        ...
        {
            case obj_bloodcherry:
                with (instance_create(x + (image_xscale * 6), y, obj_bloodcheeryspit))
                {
...
function scr_pizzagoblin_throwOLD()
{
    if (!variable_global_exists("throw_frame"))
    {
        ...
        global.throw_frame[obj_twoliterdog] = 9;
        ...
        global.throw_sprite[obj_twoliterdog] = spr_twoliter_tipover;
        ...
        global.reset_timer[obj_tank] = 400;
