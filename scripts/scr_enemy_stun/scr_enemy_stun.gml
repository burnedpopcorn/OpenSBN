function scr_enemy_stun()
{
    ...
    else if (floor(image_index) == (image_number - 1) && sprite_index == spr_null)
    {
    ...
    else if (thrown)
    {
        ...
        with (create_blur_afterimage(x, y, sprite_index, image_index, image_xscale, image_yscale, image_angle))
            playerid = other.id;
    }
    ...
    if ((grounded || place_meeting(x, y + 1, obj_water)) && elite)
        stunned -= 5;
    
    if (stunned <= 0 && (grounded || place_meeting(x, y + 1, obj_water)))
    {
        vsp = 0;
        image_index = 0;
    ...
    else
    {
        ...
        else if (place_meeting(x, y + 10, obj_water) && vsp >= 0)
